...
const AboutUsContainer = styled(Container)`
  background: ${({ theme }) => (theme === 'dark' ? 'rgba(18, 18, 18, 0.8)' : 'rgba(248, 249, 250, 0.8)')};
  padding: 3rem 1.5rem;
  border-radius: 20px;
...
const CustomCard = styled(Card)`
  background: ${({ theme }) => (theme === 'dark' ? 'rgba(30, 30, 30, 0.8)' : 'rgba(255, 255, 255, 0.8)')};
  ...
  .icon {
    font-size: 3rem;
    margin-bottom: 1rem;
    color: ${({ theme }) => (theme === 'dark' ? '#667eea' : '#764ba2')};
  }
`;
...
const ReviewsSection = styled.section`
  padding: 3rem 1.5rem;
  background-color: ${({ theme }) => (theme === 'dark' ? '#181818' : '#f1f1f1')};
  text-align: center;
